...
export class LayoutContainerNameCompletionProvider extends XmlSuggestionProvider<CompletionItem> {
  ...
  public getAttributeValueConditions(): CombinedCondition[] {
    return [
      [new ElementNameMatches('referenceContainer'), new AttributeNameMatches('name')],
  ...
  public getSuggestionItems(value: string, range: Range, document: TextDocument): CompletionItem[] {
    ...
    if (layoutIndexData) {
      for (const name of layoutIndexData.getAllContainerNames(area)) {
        names.add(name);
      }
    }

    if (pageLayoutIndexData) {
      for (const name of pageLayoutIndexData.getAllContainerNames(area)) {
        names.add(name);
      }
    }
